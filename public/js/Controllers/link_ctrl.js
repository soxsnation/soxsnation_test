...
angular.module('soxsnationApp')
	.controller('LinkController', ['$scope', '$http', '$location', 'soxsAuth', 'soxsFactory',
		function LinkController($scope, $http, $location, soxsAuth, soxsFactory) {
			$scope.mode = 'none';
			...
			function saveLinkData() {
				...
				if ($scope.mode === 'edit') {
					...
					soxsFactory.updateData('link', link).then(function(data) {
						console.log(data);
					})

					url += 'update/link/' + $scope.link_id;
				} else if ($scope.mode === 'insert') {
					soxsFactory.insertData('link', link).then(function(data) {
						console.log(data);
					})
					url += 'insert/link'
				}
			...
			// 	$scope.links = data;
			// });
			$scope.names = soxsFactory.names();
			soxsFactory.getData('link').then(function(links) {
				$scope.links = links;
			})
