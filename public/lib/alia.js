...
}(window.alia = window.alia || {}));;/*jshint bitwise: false*/
(function(alia, undefined) {
    "use strict";
...
}(window.alia = window.alia || {}));;(function(alia, undefined) {
    "use strict";
...
(function($, alia) {
    ...
    alia.defineControl({
        ...
        return function(options) {
            ...
            if (alia.isAccessor(options.loading)) {
                ...
                // Append button component
                html = '<button alia-context type="button" class=":class"></button>';
                elm = this.append(html, {
                    class: cls.join(' ')
                });
    ...
    alia.defineControl({
        ...
        function doOption(elm, opts, i) {
            var opt = alia.doOption(elm, {
                option: opts.at(i)
            });
            opts.at(i).onUnresolve(function() {
                $('#' + opt.id()).remove();
                $('#' + elm.id()).trigger('change');
            });
            opt.value.onResolve(function() {
                $('#' + elm.id()).trigger('change');
            });
        }
        ...
        return function(options) {
            ...
            function setPools() {
                ...
                for (var prop in pools) {
                    if (pools.hasOwnProperty(prop)) {
                        engine.add(pools[prop]);
                    }
                }
            }

            function makePoolHandler(idx) {
                return function(value) {
            ...
            if (alia.isAccessor(options.pool)) {
                ...
                for (var i = 0; i < options.pool.length; ++i) {
                    if (alia.isAccessor(options.pool[i])) {
                        options.pool[i].onResolve(makePoolHandler(i));
                    }
                }
            }
    ...
    alia.defineControl({
        ...
        return function(options) {
            ...
            var elm = this.append(alia.replace(content, {
                text: '<img src=":source" alt=":alt" class=":class">'
            }), {
...
}($, alia));;(function($, alia, hljs, _, Markdown, MathJax) {
    "use strict";
...
}($, alia, hljs, _, Markdown, MathJax));;(function($, alia) {
    "use strict";
    ...
    alia.defineControl({
        ...
        function makeArgChange(set, index, render) {
            return function(value) {
                set(index, value);
                render();
            };
        ...
        return function(options) {
            ...
            for (var i = 0; i < options.args.length; ++i) {
                if (alia.isAccessor(options.args[i])) {
                    ...
                    options.args[i].onResolve(makeArgChange(set, i, render));
                } else {
